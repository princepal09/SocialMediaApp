...
import { Heart, MessageCircle, Trash2, User2, Send } from "lucide-react";
...
const FeedPost = ({ post }: FeedPostProps) => {
  ...
  return (
    <section className="w-full">
      <article className="overflow-hidden text-white sm:rounded-2xl sm:border sm:border-black/10">
        {/* ================= HEADER ================= */}
        ...
        <div className="flex items-center justify-between px-4 py-3 sm:px-5">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <Link to={`/profile/${post?.owner?.username}`} className="shrink-0">
              <div className="h-10 w-10 overflow-hidden rounded-full bg-gradient-to-br from-violet-500 to-purple-700 p-[2px]">
                <div className="flex h-full w-full items-center justify-center overflow-hidden rounded-full bg-black">
                  {post?.owner?.profileImage ? (
                    ...
                    <User2 size={19} className="text-zinc-300" />
                  )}
                </div>
              </div>
            </Link>
            ...
            <div className="min-w-0">
              <Link to={`/profile/${post?.owner?.username}`} className="block">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className={`group flex items-center gap-2 transition ${
                isLikedByMe ? "text-pink-500" : "text-white hover:text-pink-500"
              }`}
            >
              <Heart
                size={23}
                className={`transition-transform duration-200 group-hover:scale-110 ${
                  isLikedByMe ? "fill-pink-500" : ""
                }`}
              />
          ...
          <div className="border-t border-white/10 bg-white/[0.015] px-4 py-4 sm:px-5">
            ...
            <div className="mt-5 flex items-center gap-2 border-t border-white/10 pt-4">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && !createCommentLoading) {
                    handleAddComment();
                  }
                }}
              ...
              <button
                disabled={createCommentLoading || !commentText.trim()}
